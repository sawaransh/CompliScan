...
function historyKey() {
  return state.officer ? `compliscan.history.${state.officer.id}` : STORAGE_KEYS.history;
}
...
const state = {
  ...
  photos: [],   // [{ file, dataUrl, name }] — bulk upload + live capture, max 6
...
async function navigateToTab(screen) {
  if (screen === "home") await renderHome();
  if (screen === "history") await renderHistory();
  if (screen === "reports") await renderReports();
...
function migrateStoredHistory() {
  // Legacy global history (pre-MongoDB) is scoped per-officer; clear the shared key to avoid cross-officer leakage.
  try { localStorage.removeItem(STORAGE_KEYS.history); } catch (e) {}
  // Per-officer cache is kept only as offline fallback — server is source of truth.
}

function mapScanDoc(entry) {
  // MongoDB scan doc → officer history entry shape used by renderers
  return {
    id: entry.inspection_id || entry.run_id,
    runId: entry.run_id,
    date: entry.date,
    product: entry.product || "Unnamed product",
    brand: entry.brand,
    city: entry.city,
    status: entry.status,
    ...
    reportGenerated: !!entry.report_generated,
    imageCount: entry.image_count || 0,
    raw: entry.result ? { inspection_id: entry.inspection_id, compliance: entry.result.compliance, images: entry.result.images, disclaimer: entry.result.disclaimer, legal_notice: entry.result.legal_notice, warnings: entry.result.warnings } : null,
    _doc: entry,
  };
}
...
async function syncHistoryFromServer() {
  ...
  try {
    const res = await authFetch("/api/officer/scans?limit=200");
    const data = await res.json();
    const docs = Array.isArray(data.scans) ? data.scans : [];
    state.history = docs.map(mapScanDoc);
    // Cache per-officer for offline viewer (stripped of heavy boxes, just markers)
    try { localStorage.setItem(historyKey(), JSON.stringify(state.history.map((h) => ({ ...h, raw: null })))); } catch (e) {}
    state.settings.lastSync = new Date().toISOString();
    writeJSON(STORAGE_KEYS.settings, state.settings);
  } catch (e) {
    // Offline fallback: load per-officer cache if server unreachable
    const cached = readJSON(historyKey(), null);
    if (Array.isArray(cached) && cached.length) state.history = cached;
  }
}
...
async function enterApp() {
  ...
  showScreen("home");
  tryGeolocate();
  // Keep officer view live: resync when tab regains focus
  window.addEventListener("focus", async () => {
    await syncHistoryFromServer();
    const active = document.querySelector(".screen.active");
    if (!active) return;
    if (active.id === "screen-home") await renderHome();
    if (active.id === "screen-history") await renderHistory(document.querySelector("#history-filters .filter-chip.active")?.dataset.filter || "all");
    if (active.id === "screen-reports") await renderReports();
  });
}
...
function wireStaticEvents() {
  ...
  $("#qa-history").addEventListener("click", async () => { await renderHistory(); showScreen("history"); });
  $("#qa-reports").addEventListener("click", async () => { await renderReports(); showScreen("reports"); });

  // Capture
  $("#capture-back").addEventListener("click", () => navigateToTab(state.resultOrigin === "history" ? "history" : "home"));
  $("#btn-upload-photos").addEventListener("click", () => $("#bulk-input").click());
  $("#btn-live-capture").addEventListener("click", () => $("#camera-input").click());
  ...
  $$("#history-filters .filter-chip").forEach((chip) => {
    chip.addEventListener("click", async () => {
      ...
      await renderHistory(chip.dataset.filter);
    });
  });
  $("#history-clear").addEventListener("click", async () => {
    if (!state.history.length) return;
    if (confirm("Clear your scan history view on this device? (Server records remain for supervisor)")) {
  ...
  // Profile
  $("#profile-sync").addEventListener("click", async () => {
    await syncHistoryFromServer();
    renderProfile();
    await renderHome();
...
async function renderHome() {
  ...
  // Ensure latest from server before painting stats
  await syncHistoryFromServer();

  // Prefer server overview for accurate counts (fallback to local filter)
  let scansThisMonth = 0, flaggedThisMonth = 0, pendingCount = 0;
  try {
    const ov = await (await authFetch("/api/officer/overview")).json();
    pendingCount = ov.pending || 0;
    // derive month slice locally for scans/flagged; total pending from server
    const now = new Date();
    const monthEntries = state.history.filter((h) => {
      const d = new Date(h.date);
      return d.getMonth() === now.getMonth() && d.getFullYear() === now.getFullYear();
    });
    scansThisMonth = monthEntries.length;
    flaggedThisMonth = monthEntries.filter((h) => h.status === "NON-COMPLIANT").length;
  } catch (e) {
    const now = new Date();
    const monthEntries = state.history.filter((h) => {
      const d = new Date(h.date);
    ...
    scansThisMonth = monthEntries.length;
    flaggedThisMonth = monthEntries.filter((h) => h.status === "NON-COMPLIANT").length;
    pendingCount = state.history.filter((h) => !h.submitted).length;
  }

  $("#stat-scans").textContent = scansThisMonth;
  $("#stat-flagged").textContent = flaggedThisMonth;
...
function startCaptureFlow(origin) {
  ...
  renderPhotoStrip();
  updateCaptureState();
  $("#capture-hint").textContent = "Add up to 6 photos of the package — front, back and close-ups of small print.";
  showScreen("capture");
}
...
async function addPhotoFiles(fileList) {
  for (const file of fileList) {
    if (state.photos.length >= MAX_PHOTOS) {
      toast(`Maximum ${MAX_PHOTOS} photos per inspection`);
      break;
    }
    if (!file || !file.type.startsWith("image/")) continue;
    const dataUrl = await fileToDataUrl(file);
    state.photos.push({ file, dataUrl, name: file.name });
  }
  renderPhotoStrip();
  updateCaptureState();
}
...
function renderPhotoStrip() {
  ...
  if (!state.photos.length) {
    strip.innerHTML = `<div class="empty-note">No photos yet — upload or capture up to ${MAX_PHOTOS}.</div>`;
    return;
  }
  state.photos.forEach((shot, idx) => {
    const tile = document.createElement("div");
    tile.className = "extra-shot";
    tile.innerHTML = `<img src="${shot.dataUrl}" alt="Photo ${idx + 1}"><button type="button" aria-label="Remove photo ${idx + 1}">${ICONS.cross}</button>`;
...
function updateCaptureState() {
  ...
  $("#capture-count").textContent = n === 0 ? "0 images captured" : `${n} of ${MAX_PHOTOS} images selected`;
  $("#photo-count-sub").textContent = n ? `${n} / ${MAX_PHOTOS}` : "";
  $("#btn-proceed-analysis").disabled = n === 0;
}
...
/* ============================================================
    ...
    ============================================================ */
async function handleScanSuccess(data, shots) {
  ...
  // Optimistically show, then sync authoritative history from MongoDB
  // so supervisor dashboard sees it instantly.
  ...
  await syncHistoryFromServer();
  // Re-attach raw for current view after sync stripped it
  const refreshed = state.history.find((h) => h.id === entry.id);
  if (refreshed) { refreshed.raw = data; state.currentHistoryId = refreshed.id; }
}
...
async function openHistoryEntry(id, origin) {
  let entry = state.history.find((h) => h.id === id);
  if (!entry) return;
  state.resultOrigin = origin;
  state.currentHistoryId = id;
  // If raw was stripped from cache, fetch full result from server
  if (!entry.raw && entry.runId) {
    try {
      const detail = await (await authFetch(`/api/officer/scans/${entry.runId}`)).json();
      if (detail && detail.result) {
        // Reconstruct a result-shaped object compatible with renderResult
        entry.raw = {
          inspection_id: detail.inspection_id || detail.result.inspection_id || entry.id,
          compliance: detail.result.compliance,
          images: detail.result.images,
          disclaimer: detail.result.disclaimer,
          legal_notice: detail.result.legal_notice,
          warnings: detail.result.warnings || [],
          result_file: `/files/${entry.runId}/result.json`,
        };
      } else {
        // Fallback to file endpoint
        const r = await authFetch(`/api/result/${entry.runId}`);
        entry.raw = await r.json();
      }
      // Update cache entry
      const idx = state.history.findIndex((h) => h.id === id);
      if (idx >= 0) state.history[idx].raw = entry.raw;
    } catch (e) {
      toast("Could not load scan details");
      return;
    }
  }
  state.currentRun = entry.raw;
  if (!state.currentRun) { toast("Scan details unavailable offline"); return; }
...
function renderSummary(data) {
  ...
  covGrid.innerHTML = "";
  if (captured.size === 0 && images.length) {
    // Bulk/casual photos carry no side tags — show the photos themselves.
    $("#sum-cov-chip").textContent = `${images.length} photo${images.length > 1 ? "s" : ""}`;
    $("#sum-cov-chip").className = "mini-chip blue";
    images.forEach((img, i) => {
      const cell = document.createElement("div");
      cell.className = "cov-side";
      cell.innerHTML = `<div class="cov-photo"><img src="${img.annotated_url}" alt="Photo ${i + 1}"></div><span>Photo ${i + 1}</span>`;
...
function renderEvidence(images) {
  ...
  $("#ev-all-chip").textContent = captured.size ? `${captured.size} / 6 captured` : `${images.length} photo${images.length === 1 ? "" : "s"}`;
  grid.innerHTML = "";
  images.forEach((im, i) => {
    ...
    tile.innerHTML = `
      ...
      <span class="ev-tag">${side ? esc(SIDE_LABELS[side]) : `#${i + 1}`}</span>`;
...
function markCurrentReportGenerated() {
  ...
  if (entry) {
    ...
    if (entry.runId) authFetch(`/api/inspection/${entry.runId}/report`, { method: "POST" }).then(() => syncHistoryFromServer()).catch(() => {});
  }
}
...
function submitToSupervisor() {
  ...
  if (entry.runId) authFetch(`/api/inspection/${entry.runId}/submit`, { method: "POST" }).then(() => syncHistoryFromServer()).catch(() => {});
...
async function renderHistory(filter = "all") {
  ...
  // Always refresh from server before painting so officer only sees own scans
  await syncHistoryFromServer();
  ...
  emptyEl.textContent = filter === "all" ? "No scans yet — your inspections will appear here." : "No scans match this filter.";
...
async function renderReports() {
  ...
  items.forEach((h) => {
    ...
    row.addEventListener("click", async () => {
      ...
      // Ensure raw is loaded
      if (!h.raw && h.runId) {
        try {
          const d = await (await authFetch(`/api/officer/scans/${h.runId}`)).json();
          h.raw = d.result ? { inspection_id: d.inspection_id || d.result.inspection_id || h.id, compliance: d.result.compliance, images: d.result.images, disclaimer: d.result.disclaimer, legal_notice: d.result.legal_notice, warnings: d.result.warnings || [] } : h.raw;
        } catch (e) {}
      }
      state.currentRun = h.raw;
      if (!state.currentRun) { toast("Report details unavailable"); return; }
